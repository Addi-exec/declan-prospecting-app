...
- `loadOwners()` / `saveOwners(arr)` — the Owners file (known property owners; v8.0.0)
...
## Data
- Persists to `<dataDir>/prospecting-data.json`, an OBJECT `{ contacts, buyers, properties, inspections, drops, owners }`
...
- IPC handlers are generated by `makeCollectionHandlers(key, tab, headers)` in `main.js` for
  `contacts`, `buyers`, `properties`, `inspections`, `drops`, `owners` (channels `<key>:load` / `<key>:save`), each
...
- Sheet layout: six tabs — `Contacts` / `Buyers` / `Properties` / `Inspections` / `Drops` / `Owners` — columns =
  `CONTACT_HEADERS` / `BUYER_HEADERS` / `PROPERTY_HEADERS` / `INSPECTION_HEADERS` / `DROP_HEADERS` / `OWNER_HEADERS`. Generalized
...
### Owner record shape (Owners — panel `id="owners"`, v8.0.0)
```js
{ id /* 'o…' */, first, last, mobile, email,
  address /* their CONTACT address — not necessarily a property they own */,
  props /* [{ propertyId /* '' or a Properties id */, address /* used when propertyId is '' */,
             notes }] — an owner can hold a property that is NOT in the inventory, so the
             address is stored raw; GS stores the array as JSON (in GS_JSON_FIELDS) */,
  linkType /* '' | 'contact' | 'buyer' */, linkId /* the source record's id, for the jump-back button */,
  archived /* bool */, notes, createdAt /* ISO */ }
```
- Owners are a STANDALONE collection, never derived: an owner may be someone you met socially.
...
- The detail pane is `.own-split`: properties owned on the left, a live-editable `.own-notes`
  ...
  would steal focus); per-property notes save the same way via `ownerSetPropNote`.
- `ownerCapList()` caps the list at ten rows exactly like `propCapList()`/`buyerCapList()`.
- Owners appear in quick search (Ctrl/Cmd+K), the Excel export (own sheet; `NON_CONTACT_SHEETS`
  skips it on import), backup/restore, and Google Sheets sync.
...
- The properties left column is `.md-list.md-col-sticky` — a sticky flex column
  ...
  `propCapList()` sets the box's `max-height` to exactly **ten rows** (measured off the tenth
  `.md-row`, since a row is not a fixed height, and skipped while the panel is hidden — hence
  the `showPanel` re-run). That JS cap is the only height rule at any width.
...
### Owners section (v8.0.0)
Sidebar item between Buyers and Properties → panel `id="owners"`. Master–detail like the
others: `.md-list.md-col-sticky` + `.md-list-scroll` (ten-row cap) on the left, `.md-detail`
on the right; add/edit form lives in the drawer (`DRAWER_FORMS.owner`, ids `o-*`).
See the Owner record shape above for the data model and the import rules.
...
1. **Versioning** (semver from v1.0.0 on): `MAJOR.MINOR.PATCH` — **MAJOR** = big overhauls /
   ...
   `package.json` "version". Add a changelog entry on the About page. Current: **8.0.0**.
