...
## Data
- Persists to `<dataDir>/prospecting-data.json`, an OBJECT `{ contacts, buyers, properties }`
  (three arrays). Older builds stored a bare array of contacts; `readData()` in `main.js`
  auto-migrates `array → {contacts:array, buyers:[], properties:[]}` on read. Every save is a
  read-modify-write via `updateData()` so writing one collection never clobbers the others.
...
- IPC handlers are generated by `makeCollectionHandlers(key, tab, headers)` in `main.js` for
  `contacts`, `buyers`, `properties` (channels `<key>:load` / `<key>:save`), each local-first
  with optional per-tab Google Sheets sync.
...
- Sheet layout: three tabs — `Contacts` / `Buyers` / `Properties` — columns =
  `CONTACT_HEADERS` / `BUYER_HEADERS` / `PROPERTY_HEADERS`. Generalized `gsLoadTab`/`gsSaveTab`
  read/write any tab (range `<tab>!A:ZZ`); array fields (`types`,`suburbs`,`enquiries`) are stored
  comma-separated in one cell; `GS_NUM_FIELDS`/`GS_BOOL_FIELDS` coerce types on load. `ensureTabs`
  adds any missing tab (so sheets created before v0.25 gain Buyers/Properties on demand).
